 ...
 *
 * @package PSourceProjektManager\PSourceProjektManagerTemplate
 */
...
/**
 * PSourceProjektManagerTemplate is a collection of methods that are used as a template tags.
 *
 * @package PSourceProjektManager\PSourceProjektManagerTemplate
 */
class PSourceProjektManagerTemplate {
	...
	function task_view( $args = array() ) {
		...
		$task_breaker_tasks = PSourceProjektManagerTasksController::get_instance();
		...
		if ( ! empty( $search ) ) {
			...
			echo '<p id="task_breaker-view-info">' . sprintf( __( 'Suchergebnis für: "%s"', 'bp-projekt-manager' ), esc_html( $search ) ) . '</p>';

		} else {

			if ( 'no' === $show_completed ) {
				echo '<p id="task_breaker-view-info">' . sprintf( _n( 'Derzeit wird %d Aufgabe angezeigt ', 'Derzeit werden %d Aufgaben angezeigt ', $open_tasks_no, 'bp-projekt-manager' ), $open_tasks_no );
				echo sprintf( __( 'out of %d', 'bp-projekt-manager' ), $all_tasks_no ) . '</p>';
			}

			if ( $show_completed == 'yes' ) {
				echo '<p id="task_breaker-view-info">' . sprintf( _n( 'Derzeit wird %d abgeschlossene Aufgabe angezeigt ', 'Derzeit werden %d erledigte Aufgaben angezeigt ', $completed_task_no, 'bp-projekt-manager' ), $completed_task_no );
				echo sprintf( __( 'out of %d', 'bp-projekt-manager' ), $all_tasks_no ) . '</p>';
			}
		}
		...
		if ( empty( $tasks ) ) {
			...
			echo '<p class="bp-template-notice error" id="task_breaker-message">';
				echo __( 'Keine Ergebnisse gefunden. Versuche es mit einem anderen Filter oder füge eine neue Aufgabe hinzu.', 'bp-projekt-manager' );
			echo '</p>';
			...
			echo '<tr>';
				echo '<th width="70%">' . __( 'Titel', 'bp-projekt-manager' ) . '</th>';
				echo '<th>' . __( 'Priorität', 'bp-projekt-manager' ) . '</th>';
				echo '<th>' . __( 'Datum', 'bp-projekt-manager' ) . '</th>';
			echo '</tr>';
			...
			foreach ( (array) $tasks as $task ) {
				...
				$row_actions = '<div class="row-actions">';
					$row_actions .= '<span class="edit"><a href="#tasks/edit/' . intval( $task->id ) . '">'.esc_html__('Bearbeiten', 'bp-projekt-manager').'</a> | </span>';
				if ( empty( $completed ) ) {
					...
					$row_actions .= '<span data-task_id="' . intval( $task->id ) . '" class="task_breaker-renew-task"><a href="#">Aufgabe erneuern</a> | </span>';
				}
				...
				if ( '0000-00-00 00:00:00' !== $task->date_added ) {
					...
					echo '<td>' . __( 'N/A','bp-projekt-manager' ) . '</td>';
				}
			...
			echo '<span class="displaying-num">' . sprintf( _n( '%s Aufgabe', '%s Aufgaben', $total, 'bp-projekt-manager' ),$total ) . '</span>';

			if ( $total_page >= 1 ) {
				echo '<span id="trive-task-paging" class="pagination-links">';
					echo '<a class="first-page disabled" title="' . __( 'Gehe zur ersten Seite', 'bp-projekt-manager' ) . '" href="#tasks/page/' . $min_page . '">«</a>';
					echo '<a class="prev-page disabled" title="' . __( 'Gehe zur vorherigen Seite', 'bp-projekt-manager' ) . '" href="#">‹</a>';

							echo '<span class="paging-input"><label for="task_breaker-task-current-page-selector" class="screen-reader-text">' . __( 'Seite auswählen', 'bp-projekt-manager' ) . '</label>';
					...
					echo '<a class="next-page" title="' . __( 'Zur nächsten Seite gehen', 'bp-projekt-manager' ) . '" href="#">›</a>';
					echo '<a class="last-page" title="' . __( 'Uur letzten Seite gehen', 'bp-projekt-manager' ) . '" href="#tasks/page/' . absint( $max_page ) . '">»</a></span>';
				echo '</span>';
			}
	...
	function display_project_user( $user_id = 0, $post_id = 0 ) {
		...
		if ( function_exists( 'bp_core_get_user_domain' ) ) {
			...
			esc_html_e( 'Begonnen von ', 'bp-projekt-manager' ); ?>

			<a href="<?php echo esc_url( $user_profile_url ); ?>" title="<?php esc_attr_e( 'Besuche das Benutzerprofil', 'bp-projekt-manager' ); ?>">
		...
		if ( function_exists( 'groups_get_group' ) ) {
			...
			if ( ! empty( $group->id ) ) {
				...
				esc_html_e( 'unter &raquo;', 'bp-projekt-manager' ); ?>
	...
	function display_new_project_modal_button() {
		if ( is_user_logged_in() ) { ?>
			<a id="task_breaker-new-project-btn" class="<?php echo esc_attr( apply_filters( 'task_breaker_new_project_modal_button_class', 'button' ) ); ?>" href="#">
			    <?php esc_html_e( 'Neues Projekt', 'bp-projekt-manager' ); ?>
			</a>
		<?php
		}
	}
