...
/**
 * PSourceProjektManagerCore is a collection of methods that are useful
 * across the whole application
 *
 * @package PSourceProjektManager\PSourceProjektManagerCore
 */
class PSourceProjektManagerCore {
	...
	function get_component_name() {
		return apply_filters( 'task_breaker_component_name', __( 'Projekte', 'bp-projekt-manager' ) );
	}
	...
	function count_tasks( $project_id = 0, $type = 'all' ) {
		...
		$task_breaker_tasks = new PSourceProjektManagerTask();
	...
	function get_user_group_admin_mod() {
        ...
        $group_member_project_creation = apply_filters( 'taskbreaker/core/functions/PSourceProjektManagerCore/get_user_group_admin_mod/project-creation-for-group-member', __return_false() );
	...
	public function get_displayed_user_groups_projects() {
		...
		if ( ! function_exists('groups_get_user_groups') ) {
			return array(
				...
				'total_pages' => 1,
				'total_user_groups' => 0,
				'summary' => 'Gehe zu Deiner WordPress-Verwaltungsseite (wp-admin), gehe zu "Einstellungen"> "BuddyPress" und aktiviere "Benutzergruppen", um die Projektverwaltungsfunktion zu aktivieren.',
			);
		} else {
			$user_groups = groups_get_user_groups( $user_id );
		}
		...
		return array(
				...
				'total_user_groups' => $user_groups['total'],
				'summary' => sprintf(
					esc_html__( 'There are a total of %s projects found in the %s group(s) of which %s is a member.', 'bp-projekt-manager' ),
					'<strong>' . $total . '</strong>',
					'<strong>' . $user_groups['total'] . '</strong>',
	...
	public function get_group_projects( $group_id ) {
		...
		return array(
				...
				'summary' => sprintf(
					esc_html__( 'In dieser Gruppe wurden insgesamt %s Projekte gefunden.', 'bp-projekt-manager' ),
