 ...
 *
 * @package PSourceProjektManager\PSourceProjektManagerCore\FileAttachments
 */
 ...
 */
class PSourceProjektManagerFileAttachment {
	...
	public function process_http_file() {
		...
		if ( ! is_user_logged_in() ) {
			return array( 'error' => __( 'Authentifizierungsprobleme. Beenden...', 'bp-projekt-manager' ) );
		}
		...
		if ( empty( $file ) ) {
			return array( 'error' => __( 'Habe keine http-Datei erhalten. Beenden...', 'bp-projekt-manager' ) );
		}

		if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
			return array( 'error' => __( 'Es ist ein Fehler aufgetreten. Bitte überprüfe die maximale Größe und die maximale Headergröße.', 'bp-projekt-manager' ) );
		}
		...
		if ( $fs->delete( $tmp_dir, true ) ) {
			...
			if ( $fs->mkdir( $tmp_dir ) ) {
				...
				return array( 'error' => __( 'Temporäres Verzeichnis kann nicht erstellt werden. Berechtigungsfehler.', 'bp-projekt-manager' ) );
			}

		} else {
			return array( 'error' => __( 'Temporäres Verzeichnis kann nicht gelöscht werden. Berechtigungsfehler.', 'bp-projekt-manager' ) );
		}

		return array( 'error' => __( 'Datei-Upload nicht möglich.', 'bp-projekt-manager' ) );
