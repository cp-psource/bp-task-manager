...
final class Task_Breaker_BP_Mail_Register {
	...
	function new_task_email_template() {
		...
		$post_exists = post_exists( '[{{{site.name}}}] Dir ist eine neue Aufgabe zugewiesen worden.' );
		...
		$email_content = '';
		$email_content .= "Dir ist eine neue Aufgabe zugewiesen worden.\n";
		$email_content .= "\n<a href=\"{{{task.url}}}\">Bring mich zu meiner neuen Aufgabe</a>";

		// Create post object.
		$task_update_email_post = array(
			 'post_title' => __( '[{{{site.name}}}] Es wartet eine neue Aufgabe auf Dich.', 'bp-projekt-manager' ),
		...
		if ( $post_id ) {
			   ...
			   $tt_ids = wp_set_object_terms( $post_id, 'user_assigned_a_task', bp_get_email_tax_type() );
				...
				wp_update_term(
					...
					'description' => __( 'Ein Mitglied eines Projekts erhielt eine neue Anfrage.', 'bp-projekt-manager' ),
					)
				);
	...
	function new_task_comment_email_template() {
		...
		$email_content = '';
		$email_content .= "<a href=\"{{{user.url}}}\">{{user.display_name}}</a> hat ein neues Update in der Aufgabe veröffentlicht.\n";
		$email_content .= "\n<a href=\"{{{task.url}}}\">Gehe zur Aufgabendiskussion</a> um zu antworten oder das Gespräch nachzuholen.";

		// Create post object.
		$task_update_email_post = array(
			 'post_title' => __( '[{{{site.name}}}] Neues Aufgaben-Update.', 'bp-projekt-manager' ),
			 'post_content' => $email_content, // HTML email content.
			 'post_excerpt' => $email_content, // Plain text email content.
			 'post_status' => 'publish',
			 'post_type' => bp_get_email_post_type(), // This is the post type for emails.
		...
		if ( $post_id ) {
			   ...
			   $tt_ids = wp_set_object_terms( $post_id, 'task_received_an_update', bp_get_email_tax_type() );
				...
				wp_update_term(
					absint( $term->term_id ), bp_get_email_tax_type(), array(
					'description' => __( 'Ein Mitglied eines Projekts hat eine Aufgabe aktualisiert.', 'bp-projekt-manager' ),
					)
				);
