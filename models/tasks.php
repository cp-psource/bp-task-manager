<?php
/**
 * PSourceProjektManagerTask
 *
 * This model contains the object used for PSourceProjektManager
 *
 * @author  dunhakdis
 * @since   1.0
 * @version 1.1
 */
class PSourceProjektManagerTask {
	...
	*
	...
	public function __construct() {
		...
		// Set the use access to the instance of PSourceProjektManagerCT.
		$this->user_access = PSourceProjektManagerCT::get_instance();
	...
	public function getPriorityCollection() {
		return array(
		 '1' => apply_filters( 'task_breaker_task_priority_1_label', __('Normal','bp-projekt-manager') ),
		 '2' => apply_filters( 'task_breaker_task_priority_2_label', __('Hoch','bp-projekt-manager') ),
		 '3' => apply_filters( 'task_breaker_task_priority_3_label', __('Kritisch', 'bp-projekt-manager') ),
		);
	}
		...
		if ( ! empty( $this->id ) ) {
			...
			if ( $dbase->insert( $this->model, $args, $format ) ) {
				...
				if ( function_exists( 'bp_activity_add' ) ) {
					...
					$action = sprintf( __( '%1$s neue Aufgabe hinzugefügt unter %2$s', 'bp-projekt-manager' ), $bp_user_link, $task_breaker_project_name );
	...
	public function getTaskStatistics( $project_id = 0, $task_id = 0 ) {
		...
		$stats = array(
		  ...
		  'status'    => null,
		  'progress'  => sprintf( __( '%s Abgeschlossen', 'bp-projekt-manager' ), $task_progress ),
		 );
		...
		if ( $task_id > 0 ) {
			...
			if ( $the_task ) {
				...
				$task_status = __( 'Offen', 'bp-projekt-manager' );

				if ( $completed_by >= 1 ) {

					$task_status = __( 'Abgeschlossen', 'bp-projekt-manager' );
