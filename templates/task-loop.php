<?php $task_breaker_tasks = PSourceProjektManagerTasksController::get_instance(); ?>

<?php $core = new PSourceProjektManagerCore(); ?>

<?php $task_user_access = PSourceProjektManagerCT::get_instance(); ?>
...
<?php if ( $task_user_access->can_see_project_tasks( intval( $args['project_id'] ) ) ) { ?>
						...
						if ( $deadline > 0 ) 
						{
							...
							<span class="deadline-human-time-diff">
						 		<em>
						 			...
						 			<?php } else { ?>
						 				<strong>
						 					<?php esc_html_e('Past Due', 'bp-projekt-manager'); ?>
						 				</strong>
						 			<?php } ?>
						 			</small>
						 		</em>
	...
	<?php } else { ?>
		<div class="task-breaker-message info">
			<?php esc_html_e( 'Keine Aufgaben gefunden. Probiere verschiedene Schlüsselwörter und Filter aus.', 'bp-projekt-manager' ); ?>
		</div>
	<?php } ?>
		...
		<div class="tablenav">
			...
			<div class="tablenav-pages">
				...
				<span class="displaying-num">
					<?php sprintf( _n( '%s Aufgabe', '%s Aufgaben', $total, 'bp-projekt-manager' ), $total ); ?>
				</span>
				...
				<?php if ( $total_page >= 1 ) { ?>
					...
					<span id="task_breaker-task-paging" class="pagination-links">
						...
						<a class="first-page disabled" title="<?php esc_attr_e( 'Gehe zur ersten Seite', 'bp-projekt-manager' );?>" href="#tasks/page/'.$min_page.'">«</a>
						<a class="prev-page disabled" title="<?php esc_attr_e( 'Gehe zur vorherigen Seite', 'bp-projekt-manager' );?>" href="#">‹</a>

						<span class="paging-input">
							<label for="task_breaker-task-current-page-selector" class="screen-reader-text">
								<?php esc_html_e( 'Seite auswählen', 'bp-projekt-manager' ); ?>
							</label>
							...
							<?php esc_attr_e( 'von', 'bp-projekt-manager' ); ?>
						...
						<a class="next-page" title="<?php esc_attr_e( 'Zur nächsten Seite gehen', 'bp-projekt-manager' ); ?>" href="#">›</a>
						<a class="last-page" title="<?php esc_attr_e( 'Zur letzten Seite gehen', 'bp-projekt-manager' );?>" href="#tasks/page/<?php echo intval( $max_page ); ?>">»</a>
...
<?php } else { ?>
	<p class="info" id="message">
		<?php esc_attr_e( 'Nur Mitglieder dieser Gruppe können Aufgaben sehen.', 'bp-projekt-manager' ); ?>
	</p>
	<p>
		<a class="button" href="#">
			Request Access @Todo
