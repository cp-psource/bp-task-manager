...
$taskbreaker_transactions = new PSourceProjektManagerTransactions();

/**
 * Holds the methods for our Transactions. This class acts as our middleware between each post and request.
 *
 * @package TasksBreaker\PSourceProjektManagerTransactions
 */
class PSourceProjektManagerTransactions {
	...
	public function transactions_callblack() {
		...
		if ( ! wp_verify_nonce( $nonce, 'task_breaker-transaction-request' ) ) {
			...
			esc_html_e( 'Ungültige Anfrage. Deine Sitzung ist bereits abgelaufen (ungültige Nonce). Bitte gehe zurück und aktualisiere Deinen Browser. Vielen Dank!', 'bp-projekt-manager' );
			return;
		...
		if ( in_array( $method, $allowed_callbacks, true ) ) {
			...
			if ( file_exists( $method_module ) ) {
				...
				(
					array(
						'message' => sprintf( __( 'Die Route kann nicht gefunden werden: %s', 'bp-projekt-manager' ), $method ),
					)
				);
			}
		} else {
