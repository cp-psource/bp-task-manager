<?php
/**
 * This file contains the PSourceProjektManagerProjectsComponent
...
/**
 * PSourceProjektManagerProjectsComponent
 *
 ...
 */
class PSourceProjektManagerProjectsComponent extends BP_Component {
	...
	*
	...
	function __construct() {
		...
		$this->core = new PSourceProjektManagerCore();
	 ...
	 * PSourceProjektManagerProjectsComponent are listed here
	 *
	 * @uses   buddypress()
	 * @return void
	 */
	...
	public function setup_globals( $args = array() ) {
		...
		$globals = array(
			...
			'root_slug' => isset( buddypress()->pages->{$this->id}->slug ) ? buddypress()->pages->{$this->id}->slug : BP_PROJECTS_SLUG,
		 	'has_directory' => true,
		 	'directory_title' => __( 'Projekte', 'bp-projekt-manager' ),
		 	'search_string' => __( 'Projekte suchen...', 'bp-projekt-manager' ),
		);
	...
	function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		$main_nav = array(
			'name' => sprintf( esc_html( 'Projekte %s', 'bp-projekt-manager' ), '<span class="count">' . absint( $count ) . '</span>' ),
			'slug' => $this->id,
			'position' => 80,
			'screen_function' => array( 'PSourceProjektManagerProjectScreens', 'bp_projects_main_screen_function' ),
		...
		$sub_nav[] = array(
			'name'            => sprintf( __( '%s Projekte', 'bp-projekt-manager' ), $displayed_user_name ),
			'slug'            => 'all',
		...
		// Edit subnav.
		$sub_nav[] = array(
			'name'            => __( 'Neues Projekt', 'bp-projekt-manager' ),
			'slug'            => 'new',
			'parent_url'      => bp_loggedin_user_domain() . '' . $this->id . '/',
			'parent_slug'     => 'projects',
			'screen_function' => array( 'PSourceProjektManagerProjectScreens', 'bp_projects_main_screen_function_new_project' ),
			'position'        => 10,
		);
 ...
 * @return void
 */
buddypress()->projects = new PSourceProjektManagerProjectsComponent;
